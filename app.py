...
def inference_loop(model, device, half):
    ...
    # Fallback to GStreamer if standard open fails for a file path
    if not cap.isOpened() and isinstance(VIDEO_SOURCE, str):
        print("[inference] Standard open failed. Trying GStreamer...", flush=True)
        gst = f"filesrc location={VIDEO_SOURCE} ! decodebin ! videoconvert ! appsink"
